...
## List page — list & import

The list page uses two tabs (`role="tablist"`, persisted in **`purchase-orders:history-filters:v2`** as **`activeTab`**):

| Tab | Default | Contents |
| --- | --- | --- |
| **List** | Yes | Filters, combined payment, inline-edit grid (no count badge — filtered row count is not a tab label) |
| **Create / Import** | No | CSV/XLSX import form |

Switching tabs does not reload the list; import still refreshes the grid after a successful upload.
...
- **Sort:** Click any list data column (except the checkbox and **Actions**) to sort via `GET /api/v1/purchase-orders?sort_by=&sort_dir=`. First click on a column sorts descending; clicking the active column toggles ascending. Default: **ordered desc**. Nullable date/money columns keep nulls last. **Status** follows workflow order (Draft → Ordered → Shipped → Received → On shelves). **Total** is product + shipping + surcharge (all-null totals last). Internal `sort_by=filter` (product/price-research PO picker) is unchanged.

Derived **status labels** mapping internal enum → human copy (`poStatusLabel`).

**List table columns** (after filters): ID (+ supplier order ID subline; optional note subline when `notes` is set), **Status**, **Shipment** (method plus asynchronously resolved tracking numbers), **Created** / **Ordered** / **Estimated arrival** / **Received** / **On shelves**, **Vendor**, **Items**, **Product total**, **Shipping total**, **Surcharge total**, **Total**, **Actions** (**Delete** per row).

- **Inline edit** on the list grid (`PoHistoryRow.vue`): **Shipment** (Air / Sea / —), **Estimated arrival**, **Received**, **On shelves**, **Shipping total**, **Surcharge total**. Each change PATCHes **`PATCH /api/v1/purchase-orders/{uuid}`** with that field only (empty date/money/shipment → `null`). The saved **`PurchaseOrderResource`** is merged into the visible row so dependent cells update without a full reload:
  - **Received** / **On shelves** dates recalculate **Status** (`on_shelves` > `received` > `shipped` > `ordered` > `draft`).
  - **Shipping total** / **Surcharge total** recalculate **Total** (product + shipping + surcharge).
- **Delete** opens a destructive **`ConfirmDialog`**; on confirm calls **`DELETE /api/v1/purchase-orders/{uuid}`** (same guards as PO detail — blocked when received inventory or FIFO lots exist). Reloads the list table on success.

| Column       | API / logic                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Shipment** | `shipment_method` on **`purchase_orders`** (`air` \| `sea` \| null). Set on import/create form, **history-row inline select**, PO header edit, or inferred once from line products when unset (unambiguous air-only or sea-only). **Plamod** defaults to **air** on new POs; operator can change to sea.                                                                                                                                                                                                                                                                                                                                                                          |
...
| **`product_total`**, **`shipping_total`**, **`surcharge_total`**                                                                                              | Decimal text inputs (**CAD**). **Always before tax** — use invoice/CSV pre-tax subtotal and unit price, never the tax-inclusive grand total.                                                                                                     |
...
| **`shipment_method`**                                                                                                                                         | Optional **`air`** or **`sea`**. **Plamod** new POs default to **air** (import, restock draft, create-from-products) unless the operator picks **sea**.                  |
...
List rows include selection checkboxes. Selecting at least two POs exposes **Record combined
...
**`exclude_from_latest_arrivals_ordering`** (checkbox on edit): when **true**, the PO keeps **`received_date`** for inventory/lots but is **skipped** when building storefront Latest Arrivals collection order (`LatestArrivalCatalogOrderService`). Use for restock invoices that should not bump products ahead on the homepage. Shared SKUs fall back to their next eligible received PO. The **Push to Shopify — Latest Arrivals order** dialog shows an amber warning when this flag is on (products/tags still sync; this PO does not rank on the homepage). Re-run **Push to Shopify — Latest Arrivals order** on a received PO after toggling the checkbox off and saving to refresh Shopify collection sort.

After a successful push finalize, **`ShopifyLatestArrivalsCollectionReorderService`** reorders the **`latest-arrivals`** smart collection and **waits for Shopify’s async reorder job** (up to **`SHOPIFY_LATEST_ARRIVALS_REORDER_JOB_WAIT_SECONDS`**, default 120s) before marking the push complete. Manual refresh without re-pushing all products: **`php artisan shopify:latest-arrivals-collection-reorder`**. The homepage Latest Arrivals section uses that Shopify manual collection order. Storefront **New arrivals** / **View all** links go to **`/collections/model-kits?ovs_mk_latest_arrival=y`** (Latest arrival = Yes). When that filter is Yes, the model-kits grid uses the same catalog order as the homepage (not PO received-date sort). **`/collections/latest-arrivals`** redirects to that prefiltered hub URL.
...
**Plamod order-details CSV:** **Qty Filled** maps to **`qty_shipped`** only. **`qty_received`** stays **null** on import (no PO-linked inventory lots). Receipt is recorded later via inventory check apply, manual line edits, or explicit **Qty received** columns in generic import CSVs. **Unit Price** / **Subtotal** (before tax) feed **`unit_cost`** / **`product_total`** — ignore Plamod **Grand Total** and **Line Total (After Tax)**.
...
- **Requires every PO line** to have **`qty_received` entered** (`0` is valid; **`null`** means not entered); otherwise returns **422** with per-SKU issues (same rule as Prepare). Lines with received `0` add nothing to available qty.
...
| Created from Products grid           | **`POST /api/v1/purchase-orders/drafts/create-from-products`** — same-vendor selections keep that vendor; mixed/blank vendors use **Other/multi**. Sets line **`unit_cost`** from **`latest_unit_cost`** and fills header **`product_total`** / **`shipping_total`** (CAD estimates from cached costs × reorder qty)                                                                                                                                                             |
...
**Set/review preview dialog** (`PoWorkflowSetPricesDialog`): **Set/review** loads preview first. **Landed** uses the product line from the latest PO whose **`shipping_total` is not `NULL`** (`ProductLatestArrivedLandedUnitCostResolver`, ordered by `ordered_date`). An explicit shipping total of **`0.00` is valid**. CAD unit cost comes from that PO. **Shipping** uses `purchase_order_items.shipping_per_unit` when that line override is set; otherwise shipping and surcharge are allocated equally by quantity across **all lines on that PO**. This allows new products on the current unreceived PO to be priced as soon as shipping is entered. Lines with no PO where shipping has been entered land in **Missing landed cost**. **Formula:** landed × **1.5**, then the **X.99** CAD price closest to that target (ties go up). **Price updates** only include raises greater than **$1.00**; a formula difference of **$1.00 or less** stays in **No price change** (`keep_reason: diff_within_one_dollar`) because the catalog change is not worth a $1 move. Sections (in order): **New prices** (shows current selling price when the preview row has one), **Price updates** (raises only — formula higher than current by more than $1), **No price change** (already matches formula, difference is $1 or less, or current price is higher than formula; **Override** column **?** help explains that Apply skips these rows, **Current** is catalog price, and **Override** pre-fills the formula price), **Missing landed cost** (skipped unless an operator enters an override). Within each section, **new-on-PO products appear before existing** SKUs until a column header is clicked; each section table supports **client-side sort** on **SKU**, **Product**, **PO**, **Landed**, **Current**, **Mult.**, and **Proposed/Override** (asc/desc toggle per section, ▲/▼ indicator on the active column). Table headings remain visible while scrolling. Displayed multipliers below **1.45×** or above **1.60×** use bold red alert badges for review. Proposed/override price inputs accept manual CAD values (`0.00` format), highlight edited rows, recalculate the target multiplier in the **Mult.** column (`current → override`, even when both values round to the same multiplier), and **Reset** returns a row to the formula/current value. **Apply prices** writes formula rows plus any valid overrides; **Cancel** closes without changes.
...
**Selling price history** (`GET .../selling-price-history`): append-only **`product_selling_price_history`** rows written whenever a selling price actually changes. The PO detail list shows **one row per product**: **price before this PO → last price set on this PO**. Intermediate applies on the same SKU are collapsed. The list omits a collapsed row when the net difference is **$1.00 or less** (same threshold as Set/review suggestions). First-time prices (no previous) still appear. PO **Set/review → Apply prices** logs **`source: po_workflow`** with **`purchase_order_id`** set. Manual edits via **`PUT /products/{uuid}/selling-price`** (Price Research, Products) log **`source: manual`** with no PO link. Re-applying the same price does not create a duplicate history row. Product-scoped history: **`GET /products/{uuid}/selling-price-history`** (includes optional **`purchase_order_uuid`** when the change came from a PO workflow; that product log is unfiltered and not collapsed).
