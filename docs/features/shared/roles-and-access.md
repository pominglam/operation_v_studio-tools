...
1. **Loopback host** (`localhost`, `127.0.0.0/8`, `[::1]`, `::1`, optional port) → **no password**; role attribute defaults to **admin** for that request.
2. Otherwise, if external access is **disabled** or password **not configured** → **404** (hidden deployment).
3. Valid cookie → request proceeds with **`external_access_role`** resolved from cookie payload.
4. **Employee cookie** → web paths restricted; non-allowed paths → **404**. API paths restricted to allow-list → other APIs → **404** JSON `{ ok:false, error:"not_found" }`.
5. **Missing cookie + API** → **`401`** JSON `{ ok:false, error:"external_auth_required" }`.
6. **Missing cookie + browser** → redirect **`/external-login?next=...`**.
