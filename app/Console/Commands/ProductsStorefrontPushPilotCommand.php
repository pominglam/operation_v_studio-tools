...
final class ProductsStorefrontPushPilotCommand extends Command
{
    protected $signature = 'products:storefront-push-pilot
        {--push-only : Push pilot product tags only; skip collection setup}
        {--collections-only : Create/update pilot collections only; skip product push}
        {--dry-run : Print actions without calling Shopify}';

    protected $description = 'Phase 1–2 pilot: push ts:* tags for tapes/decals to Shopify and ensure smart collections.';

    /**
     * @var array<int, string>
     */
    private const PILOT_SKUS = [
        'MT-02', 'MT-03', 'MT-05', 'MT-10', 'MT-15', 'MT-20',
        'MS-03', 'MS-06',
        'ETC-03', 'ETC-04',
    ];
    ...
    public function handle(
        ...
        $pilotProducts = Product::query()
            ->whereIn('sku', self::PILOT_SKUS)
            ->orderBy('sku')
            ->get();

        if ($pilotProducts->count() !== count(self::PILOT_SKUS)) {
            $found = $pilotProducts->pluck('sku')->all();
            $missing = array_values(array_diff(self::PILOT_SKUS, $found));
            $this->error('Missing pilot SKUs in ERP: '.implode(', ', $missing));
        ...
        $pushRows = [];
        if (! $collectionsOnly) {
            $pushRows = $this->pushPilotProducts(
                $products,
                $pilotProducts,
                $classifier,
                $upsert,
        ...
        if (! $pushOnly) {
            if ($dryRun) {
                ...
                foreach (['tapes', 'decals'] as $handle) {
                    $collectionRows[$handle] = [
                        'gid' => '(dry-run)',
                        'handle' => $handle,
                ...
                $collectionRows = $collections->ensurePilotCollections();
            }
        }
        ...
        if ($failed) {
            $this->error('Verification failed for one or more pilot resources.');

            return self::FAILURE;
        }

        $this->info('Pilot push and collections verified.');

        return self::SUCCESS;
    }

    /**
     * @param  \Illuminate\Support\Collection<int, Product>  $pilotProducts
     * @return array<int, array{0: string, 1: string, 2: string, 3: string, 4: string}>
     */
    private function pushPilotProducts(
        ProductRepository $products,
        $pilotProducts,
        ProductStorefrontClassifier $classifier,
        ShopifyProductUpsertFromErpService $upsert,
