...
use App\Jobs\PurchaseOrderWorkflowExportShopifyContentFinalizeJob;
...
final class PurchaseOrderWorkflowExportShopifyContentQueueService
{
    public const string BATCH_NAME = 'po_workflow_export_shopify_content';

    private const string CACHE_PREFIX = 'po_workflow_export_shopify_content:';

    public function __construct(
        private readonly PurchaseOrderWorkflowExportShopifyContentService $exportShopify,
    ...
    {
        $preview = $this->exportShopify->preview($purchaseOrderUuid);
        $uuids = $preview['product_uuids'];
        if ($uuids === []) {
            throw new PurchaseOrderWorkflowExportShopifyContentException('No eligible products to export.');
        }
        ...
        try {
            ...
            throw new PurchaseOrderWorkflowExportShopifyContentException($e->getMessage());
        }

        $pushOptions = ShopifyProductPushOptionsDTO::contentExport()->toArray();
        ...
        $batch = Bus::batch($jobs)
            ...
            ->finally(function (Batch $batch) use ($purchaseOrderUuid): void {
                PurchaseOrderWorkflowExportShopifyContentFinalizeJob::dispatch($purchaseOrderUuid, $batch->id);
            })
            ->dispatch();
    ...
    public function pushStatus(string $purchaseOrderUuid, string $batchId): array
    {
        ...
        if (! is_array($cached) || ($cached['purchase_order_uuid'] ?? '') !== $purchaseOrderUuid) {
            throw new PurchaseOrderWorkflowExportShopifyContentException('Export batch not found for this purchase order.');
        }

        $batch = Bus::findBatch($batchId);
        if ($batch === null) {
            throw new PurchaseOrderWorkflowExportShopifyContentException('Export batch not found.');
        }
        ...
        if ($finalizeStatus === 'failed') {
            return [
                'phase' => 'failed',
                'batch' => $batchPayload,
                'message' => is_string($cached['error'] ?? null) ? $cached['error'] : 'Export finalize failed.',
            ];
        }
    ...
    public function storeFinalizeResult(string $batchId, array $result, string $finalizeStatus = 'complete'): void
    {
        ...
        if ($finalizeStatus === 'complete') {
            ...
            $cached['error'] = is_string($result['message'] ?? null) ? $result['message'] : 'Export finalize failed.';
        }
