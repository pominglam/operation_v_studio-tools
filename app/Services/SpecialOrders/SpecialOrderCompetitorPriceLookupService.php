...
final class SpecialOrderCompetitorPriceLookupService
{
    /**
    ...
    public function lookupByProductName(string $productName, ?array $siteKeys = null): array
    {
        ...
        $keys = $siteKeys ?? SpecialOrderCompetitorPriceSites::FAST_SITE_KEYS;
        ...
        foreach ($siteKeys as $siteKey) {
            $running[$siteKey] = Process::path(base_path())
                ...
                ->start([
                    $phpBinary,
                    $artisan,
                    'special-orders:competitor-price-site',
                    $siteKey,
                ]);
        }
    ...
    private function missingSiteQuote(string $siteKey, string $message): array
    {
        return [
            'site_key' => $siteKey,
            'site_name' => SpecialOrderCompetitorPriceSites::siteLabel($siteKey),
            'site_url' => SpecialOrderCompetitorPriceSites::siteUrl($siteKey),
            'status' => 'error',
            'availability' => null,
            'currency' => 'CAD',
            'price' => null,
    ...
    private function quoteToArray(PriceLookupResult $result): array
    {
        return [
            ...
            'site_url' => SpecialOrderCompetitorPriceSites::siteUrl($result->siteKey),
