...
final class SpecialOrderCustomerPricingService
{
    /** @var list<string> */
     ...
     */
    public function resolveCustomerOfferAttributes(SpecialOrder $order, array $input): array
    {
     ...
     * @return array<string, mixed>
     */
    public function resolveUpdateAttributes(SpecialOrder $order, array $input): array
    {
     ...
     * @return array<string, mixed>
     */
    private function resolveAttributesForKeys(SpecialOrder $order, array $input, array $keys): array
    {
     ...
     * @return array<string, mixed>
     */
    private function applyDepositDerivedFields(SpecialOrder $order, array $attributes): array
    {
        if (array_key_exists('deposit_amount_override_cad', $attributes) && $attributes['deposit_amount_override_cad'] !== null) {
            ...
            $derivedPercent = SpecialOrderCustomerPricing::depositPercentFromAmount(
                $customerPrice,
    ...
    {
        $multiplier = SpecialOrderCustomerPricing::normalizeMultiplier($value);
        if ($multiplier === null) {
    ...
    private function normalizeOptionalMoney(mixed $value, string $invalidMessage): ?string
    {
        ...
        $normalized = SpecialOrderCustomerPricing::normalizeMoney($value);
        if ($normalized === null) {
    ...
    private function normalizeOptionalPercent(mixed $value, string $invalidMessage): ?string
    {
        ...
        $normalized = SpecialOrderCustomerPricing::normalizePercent($value);
        if ($normalized === null) {
            throw new \InvalidArgumentException($invalidMessage);
        }
