...
{
    /**
     * @return array{rows_parsed: int, rows_upserted: int, rows_dropped: int}
     */
    public function importFromStoragePath(string $csvStoragePath): array
    {
        ...
        try {
            ...
            DB::transaction(function () use ($handle, $map, $now, &$seenSkus, &$parsed): void {
                while (($row = fgetcsv($handle)) !== false) {
                    ...
                    $seenSkus[] = $sku;
                    PlamodPreorder::query()->updateOrCreate(
                        ['sku' => $sku],
                        [
