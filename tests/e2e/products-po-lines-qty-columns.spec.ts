...
test('product PO lines drawer shows qty shipped and qty received columns', async ({
    ...
    await row.getByRole('button', { name: 'PO Lines' }).click();
    ...
    await expect(poRow.locator('td:nth-child(5)')).toHaveText('3');
    await expect(poRow.locator('td:nth-child(6)')).toHaveText('0');
});
