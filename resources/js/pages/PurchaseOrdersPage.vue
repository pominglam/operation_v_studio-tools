...
const vendor = ref<string>('Plamod');
...
async function loadVendors(): Promise<void> {
    try {
        ...
        if (vendors.value.length > 0 && !vendors.value.includes(vendor.value)) {
            vendor.value = vendors.value[0];
        }
    } catch {
        // If this fails, we still allow manual vendor entry to avoid blocking imports.
        vendors.value = [];
    }
}
...
async function loadHistory(): Promise<void> {
    ...
    try {
        const res = await api.get<Paginated<PurchaseOrderListRow>>('/api/v1/purchase-orders', {
            params: { per_page: 50, sort_dir: sortDir.value },
        });
            ...
            <div v-else class="mt-3 overflow-x-auto">
                <table class="min-w-full text-left text-sm">
                    <thead class="text-slate-600">
                        <tr>
                            <th class="px-2 py-2">
                                <button type="button" class="hover:underline" @click="toggleCreatedSort">
