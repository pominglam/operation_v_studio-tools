...
describe('PurchaseOrderDetailPage unit cost edit', () => {
    beforeEach(() => {
        vi.restoreAllMocks();
    });

    it('saves edited unit cost via purchase-order-item patch endpoint', async () => {
        ...
        patchMock.mockImplementation(async (url: string, payload: any) => {
            if (url === `/api/v1/purchase-order-items/${itemId}`) {
                return {
                    status: 200,
                    data: {
                        data: {
                            id: itemId,
                            unit_cost: Number(payload?.unit_cost).toFixed(2),
                        },
                    },
                };
            }
        ...
        const input = wrapper.get(`[data-testid="unit-cost-input-${itemId}"]`);
