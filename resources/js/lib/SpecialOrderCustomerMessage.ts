...
export const SPECIAL_ORDER_CUSTOMER_MESSAGE_PLACEHOLDERS = [
    '{product_name}',
    '{price}',
    '{deposit_percent}',
] as const;

export const DEFAULT_SPECIAL_ORDER_CUSTOMER_MESSAGE_TEMPLATE = `{product_name} — Special Order
...
export function formatSpecialOrderCustomerPriceLabel(
...
export function renderSpecialOrderCustomerMessage(
    template: string,
    input: {
        productName: string;
        customerPriceCad: string | number | null | undefined;
    ...
    const priceLabel = formatSpecialOrderCustomerPriceLabel(input.customerPriceCad);
    const depositLabel =
...
export function buildSpecialOrderCustomerMessage(input: {
    ...
    const template =
        input.template?.trim() || DEFAULT_SPECIAL_ORDER_CUSTOMER_MESSAGE_TEMPLATE;

    return renderSpecialOrderCustomerMessage(template, input);
}

export function previewSpecialOrderCustomerMessage(template: string): string {
    return renderSpecialOrderCustomerMessage(template, {
