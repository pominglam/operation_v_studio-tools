...
const selectableIds = computed(() => props.items.map((item) => item.id));

const allSelected = computed(
    () =>
        selectableIds.value.length > 0 &&
        selectableIds.value.every((id) => props.selectedIds.includes(id)),
);
...
function toggleAll(selected: boolean): void {
    emit('update:selectedIds', selected ? [...selectableIds.value] : []);
}
</script>
        ...
        <table data-testid="taxonomy-review-table" class="min-w-full border-collapse text-left">
            <thead class="sticky top-0 z-20 bg-slate-50">
                <tr class="text-xs font-semibold tracking-wide text-slate-600">
                    ...
                    <th
                        v-for="field in workshopTaxonomyFields"
                        :key="field.key"
                        class="px-3 py-3"
                    >
                        {{ field.label }}
            ...
            <tbody>
                <TaxonomyVerificationRow
                    ...
                    @series-decide="(row) => emit('series-decide', row)"
                    @toggle="toggleRow"
                />
            </tbody>
        </table>
